...
int lineaactuall = -1, columnaactuall = -1, nivel = 0, tot = 0, tat = 0, contEtiq = 1;// vbles globales, aca o en .h?
	...
	if(sonIguales((*tkn).nombre,"PROGRAM") == 1){
		...
		if(sonIguales((*tkn).nombre,"ID") != 1 )
			...
			if(sonIguales((*tkn).nombre,"PUNTOYCOMA") != 1)
				...
				cantVari = reglaDefVar(fp,tkn,env); 			//le paso el ambiente como parametro?		
				
			  if(hayDef(tkn) == 1) { //para no crear etiq innecesaria si no hay def de sup
				
				etiq = crearEtiq(contEtiq);	// -- l1	
				dsvs(etiq); //--
...
int sale = -1;
	...
	if((sonIguales((*tkn).nombre,"FUNCTION") == 1 ) || (sonIguales((*tkn).nombre,"PROCEDURE") == 1))	
		sale = 1;
...
int	band = 1, cantii, cantVari;
ItemTs *itm;	
char *str, **tdaa, *eti; //en tda se van a poner los td de los parfor
Ambiente *env;
...
//fps = fopen("t_s.txt","w");
	...
	cantVari = reglaDefVar(fp,tkn,env);
		while (band == 1){		
...
int band = 1, cantii = 0, cantVari;
ItemTs *itm;	
char *str, *tdretorno, **tdaa, *eti;
Ambiente *env;
...
//fps = fopen("t_s.txt","w");	
	...
	getTk(fp,tkn);		
		if(sonIguales((*tkn).nombre,"PARENTABRE") == 1){//si hay '(' entonces hay argumentos
			...
			setCantyTiposArgs(env,str,tdaa,cantii); //DEBERIA SETEAR ESTO AFUERA, Q PASA SI NO HAY ARG, NO HAY '('  (SETEADO POR DEFECTO ARRIBA en 0 y NULL)								
		...
		(*itm).desplz = -(cantii + 3); //seteo desplz de la variable de retorno (la func)
		
	//	printf("nombre func: %s, desplazamiento: %d\n", (*itm).nombre, (*itm).desplz); //SACAR SINO TIENE PARAMETROS NO SETEA EL DSPLZ1!!!!!!!!!!!!
...
otra:	reglaSentencia(fp,tkn,env);		
		//a la vuelta. preguntar si hay punto y coma, si hay volver a llamar a reglaSentenci		
		 if(sonIguales((*tkn).nombre,"PUNTOYCOMA") == 1){//si hay ';' quiere decir q sigue otra sentencia.
			...
			//printf("luego de la sentencia hay: %s\n", (*tkn).nombre);
			...
			if(sonIguales((*tkn).nombre,"CASE") == 1)
				...
				if(sonIguales((*tkn).nombre,"BEGIN") == 1){
					...
					if(sonIguales((*tkn).nombre,"ID") == 1){//analizo los toksiguientes puede ser id de asig o de proced
						...
						(*itm).asignado = 'Y';
						alvl((*itm).nivel, (*itm).desplz); // -- (guardo el valor del lado derecho(en el tope de la pila, reglaexpres) en la vble)											
						}	
						else{
							if(sonIguales((*tkn).nombre,"PARENTABRE") == 1){						
								...
								*cantiPa = 0;					
								...
								//falta apilar los paract antes de llamar
								llpr((*itm).etiq); // --
...
int sale = -1;
	...
	if(sonIguales((*tkn).nombre,"ELSE") == 1){
		sale = 1;
		contEtiq++; // -- lo saco asi crea la misma etiq q en if??
		eti2 = crearEtiq(contEtiq);// --
		dsvs(eti2); // -- etiq q va apunt a la sent desp d la sent else
		getTk(fp,tkn);
		
		etii = crearEtiq(contEtiq - 1); // misma etiq q cree en el metodo anter, comienz traducc del else (dsvf l_)
		nada(etii);			
...
return sale;	//hacer q retorne -1 o1 para ver si hubo o no else
}
//**********************************************************************
...
contEtiq++; // --
eti = crearEtiq(contEtiq);// --
dsvf(eti); // -- si la cond es falsa desviar a una etiq(puede ser el else, o lo q sigue desp del then) ..
reglaSentencia(fp,tkn,env);
hayelse = reglaAlter(fp,tkn,env); // si devuelve 1 hubo else, hago algo.. (etiq)

 if(hayelse == -1) // -- no hubo else
	nada(eti);
...
//tipodevuelto = crearStr("x");	
	...
	if((quees = reglaOp1(fp,tkn)) != -1){ //operador relacional (antes == 1)
...
int sale = -1;	
	...
	if((sonIguales((*tkn).nombre,"TRUE") == 1)){	
		apct("1"); // --
		setearStr(td,"BOOLEAN");
		sale = 1;
	}
	if((sonIguales((*tkn).nombre,"FALSE") == 1)){
		apct("0"); // --
		setearStr(td,"BOOLEAN");
		sale = 1;
	}	
	if((sonIguales((*tkn).nombre,"MAXINT") == 1)){
		apct("maxint"); // -- OJOOOOOOOOOOO ¿poner un nro fijo??	
...
int sale = -1;
	...
	if(sonIguales((*tkn).nombre,"OPRELACIONAL") == 1){
		if((sonIguales((*tkn).valor,"LT") == 1)) 
			sale = 1;
			
		if((sonIguales((*tkn).valor,"LE") == 1))
			sale = 2;
			
		if((sonIguales((*tkn).valor,"GT") == 1))
			sale = 3;
			
		if((sonIguales((*tkn).valor,"GE") == 1))
			sale = 4;
			
		if((sonIguales((*tkn).valor,"EQ") == 1))
			sale = 5;
			
		if((sonIguales((*tkn).valor,"NE") == 1))
			sale = 6;
	}
return sale;
}
...
char *td, *eti, *eti2;
	...
	dsvs(eti); // vuelve a evaluar la cond
	nada(eti2);
	//if(sonIguales((*tkn).nombre,"PUNTOYCOMA") != 1)
...
otroRo:	//reglaRotulo(fp,tkn,env,itm);
		//********************************
		if(sonIguales((*tkn).nombre,"ENTERO") != 1)
			errorSintactico(18);//se espera nro entero
		...
		//contEtiq++;
		eti = crearEtiq(contEtiq);
		dsvf(eti);//si no son iguales voy al otro rotulo .si son iguales  traduzco sentencia
...
esPrimerRot = 0;		
		...
		else{ // si no hay else decremento el cont de etiq q se habia aumentado
			contEtiq --;
			nada(eti); //el ultimo rotulo, y no hay else, no hago nada en el dsvf
		}	
...
char *lex;
	...
	existeElem(lex, lineaactuall, columnaactuall, env);
	itm = getItem(lex, env);
	
	if(esVariable(itm) != 1)								//preguntar aca o desp del parentesis...
	 errorSemantico(9, lineaactuall, columnaactuall, NULL, lex);
	 
	leer(); // -- (puso el valor en el tope de la pila)
	alvl((*itm).nivel, (*itm).desplz); // -- OBTENER LA POSIC DE LA VBLE (guardo el valor en la vble, ie hago la asign)
